		...
		for (int i = 0; i < paramMetas.size(); i++) {
			...
			if (StringUtils.isEmpty(value)) {// value的值是空
				if (paramMetas.get(i).isNullable()) {// 如果允许为空
					...
					parse.setResponseModel(new ResponseModel(ACK_ERR_REQUIRE_METHOD_PARAM,
				...
				if (request.getMethod().equals("GET")) {// get的方法需要转码
					try {
                        ...
                        logger.error("change get value encoding error", e);
					}
				}
				...
				if (obj == null) {
					parse.setResponseModel(new ResponseModel(ACK_ERR_SERVICE_PARAM_TYPE,
